...
(() => {
  ...
  async function plotSwingKinematics(profile, {
    ...
    if (profile.display_name) {
      text(profile.display_name, X(left)-0.1, 0.25 * S, { size: 24, weight: 700, color: c.header, va: "top" });
    }
    text(`${profile.year} Swing Kinematics, as ${profile.handedness}HB`, X(left)-0.1, 0.6 * S, { size: 15, color: c.subheader, va: "top" });
