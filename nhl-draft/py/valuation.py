...
def sgp_slopes(totals_list):
    ...
    d = pd.DataFrame(slopes)
    allt = pd.concat(totals_list, ignore_index=True)
    params = {
...
def coefficients(d, p):
    ...
    sum of its SV% and GAA effects.  Returns (coef: Series by stat, parts: stat -> {cat: coef}).
    """
    ...
    coef = pd.Series({s: sum(parts[s].values()) for s in STATS}, name="coef")
    return coef, parts


def category_values(players, parts):
    """Standings places each player contributes in each scoring category."""
    out = pd.DataFrame(0.0, index=players.index, columns=CATS)
    for stat, share in parts.items():
        for cat, k in share.items():
            out[cat] += k * players[stat]
    return out
...
def replacement_levels(players, value, league, tol=0.05, max_pass=30):
    ...
    Returns (repl: slot -> value, with UTIL = the best replacement among skater positions;
             starter: bool Series; repl_pid: slot -> replacement player; assigned: Series).
    """
    ...
    order = value.sort_values(ascending=False).index.to_numpy()
    v = value.to_numpy(float)[players.index.get_indexer(order)]          # values, best first
    elig = [tuple(pidx[s] for s in players.at[pid, "slots"] if s != "UTIL") for pid in order]
    ...
    for _ in range(max_pass):
        ...
        if not moved:
            break
    starter = pd.Series(False, index=players.index)
    repl_out, repl_pid = {}, {}
    ...
    assigned = pd.Series([positions[i] for i in asg], index=order).reindex(players.index)
...
def position_values(players, value, repl, assigned):
    ...
    """
    r = assigned.map(repl)
    return pd.DataFrame({"BestSlot": assigned, "Repl": r, "PosValue": repl["UTIL"] - r, "VORP": value - r})
